...
const errorHandler = error => {
    ...
    if (error && error.response && error.response.config.url.indexOf('pre-score') > -1) {
        return false;
    }
    // error for message "You cart has some issue, please correct it before proceeding"
        ...
        toast.error(error.response.data.result.message || Constants.GENERIC_ERROR_MSG, {
        ...
        if (preorderProduct) {} else {
            ...
            Router.push('/checkout/cart', `${NetworkManager.getLangUrlPrefix()}/checkout/cart`)
        }
        ...
        toast.error(error.response.data.status.message || Constants.GENERIC_ERROR_MSG, {
            position: toast.POSITION.TOP_CENTER
        });
    ...
    return Promise.reject({ message: errorObj.statusText, isError: true, errorObj, data })
};
...
class NetworkManager {
    static selected = {};
    static makeHeaderParam = (withToken = false) => async(context = null, extraHeaders = {}) => {
        let token;
        if (withToken) {
...
export const createPlusAxiosReq = (endpoint, baseUrl) => {
    ...
    const get = async params => {
        ...
        const response = await axios.get(withQs(url)(params), headers).catch(errorHandler);
        return response.data;
    };

    const del = async params => {
        const headers = await getHeader();
        const response = await axios.delete(withQs(url)(params), headers).catch(errorHandler);
        return response.data;
    };

    const post = async(bodyData, returned = 'data') => {
        let headers = await getHeader();
        const response = await axios.post(url, bodyData, headers).catch(errorHandler);
        return returned === 'data' ? response.data : (response.status === 200);
    };

    const put = async bodyData => {
        const headers = await getHeader();
        const response = await axios.put(url, bodyData, headers).catch(errorHandler);
        return response.data;
    };
    const postCoupn = async(bodyData, returned = 'data') => {
        let headers = await getHeader();
        const response = await axios.post(setUrl(endpoint), {}, headers).catch(errorHandler);
        return returned === 'data' ? response.data : (response.status === 200);
    };
    const delCoupn = async() => {
        const headers = await getHeader();
        const response = await axios.delete(setUrl(endpoint), headers).catch(errorHandler);
        return response.data;
    };
    setUrl(endpoint);
